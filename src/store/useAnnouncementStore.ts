...
interface AnnouncementActions {
    fetchAnnouncements: (params?: { search?: string; limit?: number; page?: number }) => Promise<void>;
    clearError: () => void;
}
...
export const useAnnouncementStore = create<AnnouncementStore>((set) => ({
    ...
    fetchAnnouncements: async (params = { limit: 50, page: 1 }) => {
        ...
        try {
            const response = await announcementService.getAnnouncementsForContact(params);
